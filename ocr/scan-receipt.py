...
ap.add_argument('-d', '--debug', default=False, action=argparse.BooleanOptionalAction, help='whether or not we are visualizing each step of the pipeline')
...
if args['debug']:
    print('--ratio', ratio, orig.shape[1], image.shape[1])
...
# or - try something else - detect white regions
...
for c in cnts:
	...
	if len(approx) == 4:
		if peri > maxPeri:
			...
			receiptCnt = approx
			if args['debug']:
				print('--- found outline', peri, approx)
		# break
